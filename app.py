...
if st.button("Genereer Route"):
    ...
    if not start or not end:
        ...
        if route_coords:
            # Maak Folium kaart
            m = folium.Map(location=[start[0], start[1]], zoom_start=5)

            # Voeg start en eind toe
            folium.Marker(location=start, popup="Start", icon=folium.Icon(color='green')).add_to(m)
            folium.Marker(location=end, popup="Eindpunt", icon=folium.Icon(color='red')).add_to(m)

            # Voeg route toe als lijn
            folium.PolyLine(locations=[(lat, lon) for lon, lat in route_coords], color="blue", weight=5).add_to(m)

            # Voeg tankstations toe met custom icon
