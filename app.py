...
tankstations = [
    ...
    ("5-7  avenue de la Résistance", 1.1438859, 42.9799648),
    ...
    ("rue du Moulin aux prêtres", -4.04556, 48.517937),
    ("rue Sébastienne GUYOT", -2.78191, 48.467224),
    ("Chemin de la Val Priout", 1.592579, 43.0),
    ("Rue Joseph SEGUIN", 4.636564, 43.70936),
    ("Centre Routier - Z,A,M, La Veuve", 4.32241, 49.037203),
    ("2 rue Edmé Mariotte", -1.716867, 48.051391),
    ...
    ("28 rue du maréchal Leclerc", -2.8290069848299, 47.8815061165075),
    ("Rue Claude Girard", 5.893588, 47.231766),
    ("rue Millepertuis - ZI duRancy", 3.361335, 46.537413),
    ("55 Rue du Marché Gare", 7.73914, 48.59473),
    ...
    ("ISOPARC - rue Régis Ramage", 0.679221, 47.257095),
    ...
    ("2ème avenue du Port de Lille", 3.03036199973309, 50.6269445278623),
    ...
    ("22 avenue Louis Blériot", 2.41074, 48.935534),
    ...
    ("Carrefour Mât de Ricca - Aire PL 2XL", 4.81405, 43.420607),
    ...
    ("72 rue du Bêle", -1.508902, 47.264551),
    ...
    ("Rue du Château - ZA Mâcon Nord", 4.84404, 46.364961),
    ...
    ("Boulevard Général Leclerc", 1.976197, 43.763295),
    ...
    ("rue de la laïta", -1.9256, 47.377072),
    ...
    ("Parc d'activité du Cormier", -0.915251, 47.031949),
    ("Boulevard périphérique intérieur", 2.3734753159, 48.900249771),
    ("54 avenue Lénine", 2.359673, 48.945542),
    ("Rue des Evens", -2.17633, 47.300771),
    ("La Morandais", -1.816211, 48.302029),
    ("rue Paul Guéry", 3.415378, 43.446125),
    ("L'Aubinière", -1.18996, 47.40402),
    ("Rue des alliés", 5.717021, 45.173658),
    ...
    ("Chemin des Battées", 3.990215, 47.50782),
    ("Rue Roland Moréno", -0.696674, 47.467976),
    ...
    ("Route d'Hérin", 3.447601, 50.331481),
    ...
    ("759 avenue des Frères Lumières", 5.385266, 43.422344),
    ...
    ("2 chemin de Saint Léonard", 4.096035, 49.224835),
    ("14 rue Robert Nau", 1.33242, 47.612811),
    ("Zone de la Brosse - Boulevard Léonard de Vinci", -1.55216, 47.158868),
    ("10 rue d'Estaubé", 0.024978, 43.220242),
    ...
    ("ZAC du Val Bréon", 2.832973, 48.726583),
    ("Rue du Jour", 2.364583, 48.758806),
    ("Rue Gatel", 0.095415, 48.44947),
    ("Rue Louis Blériot ZI Jonquière", 6.2064409, 49.2127499),
    ("rue du Bois de l'Ecu", 2.465229, 48.567426),
    ("Chemin de Tronquieres", 2.423188, 44.900379),
    ("Rue Nicéphore Niépce", -0.30314, 49.15592),
    ("EcoParc Rhénan 395, RD37", 7.762392, 48.663533),
    ("Rue du Bas Martineau", 2.41925, 48.9293),
    ("Chemin Neuf - ZAC Bièvre Dauphiné", 5.481912, 45.379287),
    ("597 Avenue Saint Maurice", 5.818121, 43.801182),
    ("ZA de la Coudrière", 0.727, 47.4513),
    ...
    ("rue du Président Lecuyer", 3.552717, 50.387872),
    ("1 rue Kléber", 2.188926, 48.900841),
    ...
    ("Les Quatre chemins, Rue René Couzinet", -0.811068, 46.444713),
    ("Boulevard périphérique exterieur", 2.3748087885, 48.900814679),
    ("impasse RAMOUNDET", 1.257975, 43.344385),
    ("D755", -1.044082, 46.902608),
    ("rue de la grande rivière", -1.976111, 48.6372222),
    ...
    ("63 rue de la Téoulère", -0.51339, 43.86374),
    ...
    ("Relais Maréchal Leclerc", 2.324772, 48.820553),
    ...
    ("Rue du Pré le Loup", 6.178643, 49.222276),
    ...
    ("Relais Vertou Grassinière (Ouest)", -1.51318, 47.1617),
    ("14 quai Français", -0.531422, 44.898503),
    ("Quartier Mondy", 5.038105, 45.0060573),
    ("10 rue Paul-Emile Victor, Zac Bressandière", -0.256567, 46.669589),
    ...
    ("Route de Mouliherne à Clefs", 0.008722, 47.521806),
    ("195 avenue du Haut Lévèque", -0.64952641608, 44.780775084),
    ("rue Roberto Cabanas", -4.438492, 48.438492),
    ("ZA des Dahinières, Route de Fougères", -0.816301, 48.084214),
    ("Avenue des Vieux Moulins", 6.115909, 45.887976),
    ("avenue Robert Schumann", 3.282154, 43.339939),
    ("ZI République Nord - Centre Routier", 0.34537644, 46.61820542),
    ...
    ("ZI ouest - rue des puits à Marne", 2.765209, 49.703905),
    ...
    ("ZI Le Poirier Penché - RN 19", 2.592409, 48.705436),
    ("20 rue de Villeneuve", 0.181547, 48.051984),
    ("Port de Limay-Porcheville", 1.742635, 48.983601),
    ("5 rue des Métiers", -1.28179, 43.666321),
    ("56 rue Léon Trostski", 3.87111559510231, 43.5815114035832),
    ("Angle boulevard Marcel Paul et avenue des Lions", -1.61662437021732, 47.2467973532067),
    ("Parc d'activité le Chêne", -2.229719, 48.126595),
    ("80 rue Jean Prouvé", 6.20985, 48.6155969),
    ("La Briqueterie", -0.586049, 48.300685),
    ("Rue marie curie", 5.0862674, 45.0472622),
    ("avenue de la Courbade", 4.435835, 43.754565),
    ("ZA La Carrère - 49 route de la Cantère", -1.018368, 43.740843),
    ("Avenue d'Aquitaine", 0.638194, 44.184881),
    ("Rue des Frères Lumières", -0.4871, 44.914306),
    ("1 rue Eugène Weiss", 4.416151, 45.448939),
    ("Rue Marcel Mérieux", 4.921162, 45.670149),
    ("120 Rue Marcel Paul", 2.52932712435722, 48.8127812363799),
    ("ZAC du haut de Wissous II, Chemin de la Croix Brisée", 2.309302, 48.722674),
    ...
    ("Chemin des Mésanges", 1.400312, 43.543716),
]
